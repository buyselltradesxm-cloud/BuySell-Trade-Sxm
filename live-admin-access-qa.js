...
(async () => {
  ...
  await page.goto(`${siteUrl}?admin-access-qa=1`, { waitUntil: "domcontentloaded", timeout: 30000 });
  ...
  if (!result.sbEnabled) errors.push("Supabase bridge is not enabled on the live site");
  if (result.userEmail) errors.push(`Expected signed-out state, found ${result.userEmail}`);
  if (!result.accountModalOpen) errors.push("Admin access did not ask a signed-out visitor to log in");
  if (result.adminModalOpen) errors.push("Admin modal opened for a signed-out visitor");
  if (!/admin/i.test(result.toast)) errors.push(`Expected admin login toast, got "${result.toast}"`);
